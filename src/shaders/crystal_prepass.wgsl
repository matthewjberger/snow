...
// The formations' contribution to the camera-space depth prepass.
//
// The specular mask is one, unlike every other surface here. These prisms are the
// one genuinely mirror-like thing in the frame, and the reflection pass reads
// that channel to decide where it is worth marching at all.
...
fn fragmentMain(input: Varyings) -> @location(0) vec4f {
    return vec4f(input.viewZ, 1.0, 0.0, 1.0);
}
